...
//!
//! This module implements crash-safe persistence of consensus fencing state.
//! The manifest MUST be updated before any protocol action that depends on
//! view/term fencing.
//!
//! # Invariants (CRITICAL for correctness)
//!
//! 1. **View Fencing**: Before accepting Prepare{view=V}, we must have
//!    `highest_view <= V` durably persisted. Reject if V < highest_view.
//!
//! 2. **Vote Persistence**: Before sending DoViewChange for view V,
//!    we must persist `voted_for_view = V`.
//!
//! 3. **Primary Fencing**: Before becoming Primary in view V,
//!    we must persist `highest_view = V`.
//!
//! 4. **Recovery Safety**: On boot, load manifest and enforce fencing
//!    against all incoming messages.
//!
//! # File Format
//!
//! The manifest is a 64-byte file with atomic update semantics:
//! - Write to `.tmp` file
//! - fdatasync
//! - Atomic rename
//! - fsync directory
...
//! |--------|--------------------| -----|---------------------------------------|
//! | 0      | magic              | 4    | "CMAN" (Chronon MANifest)             |
//! | 4      | version            | 4    | Format version (1)                    |
//! | 8      | highest_view       | 8    | Highest view ever seen (durable)      |
//! | 16     | voted_for_view     | 8    | View we voted in (0 = none)           |
//! | 24     | voted_for_node     | 4    | Node we voted for (u32::MAX = none)   |
//! | 28     | reserved_align     | 4    | Alignment padding                     |
//! | 32     | last_log_index     | 8    | Last log index at time of write       |
//! | 40     | last_log_view      | 8    | View of last log entry                |
//! | 48     | checksum           | 4    | CRC32C of bytes [0..48]               |
//! | 52     | reserved           | 12   | Reserved for future use               |
